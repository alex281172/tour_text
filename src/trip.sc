theme: /Trip
    ...
    #введена дата - сохраняем ее
        state: Date
            q: * @duckling.date *
            script: $session.tripDate = $parseTree.value;
            go!: /Trip/Step4
    ...
    state: Confirm
        script:
            ...
            if ($session.noDate) form += " Дата начала поездки: " + $session.noDate + "\n";
                else form += " Дата начала поездки: " + $session.tripDate.day + "." + $session.tripDate.month + "." + $session.tripDate.year + "\n";
...
theme: /SendMail
    ...
    #если результат не ОК - тоже сообщаем об этом и идем на мягкий выход
            ...
            a: Упс. Ваша заявка не отправилась. Для подбора тура обратитесь в JustTour по телефону 8-812-000-0000.
            go!: /End