theme: /Weather 
    ...
    #иначе, если нет ни города, ни страны - спрашиваем о них
    ...
    #выдаем полный ответ про погоду
            a: Погода в {{capitalize($nlp.inflect($session.place.name, "loct"))}} на {{$temp.answerDate}}: {{($temp.weather.descript).toLowerCase()}}, {{$temp.weather.temp}}°C. Ветер {{$temp.weather.wind}} м/с, порывы до {{$temp.weather.gust}} м/с.
    ...
    state: Step6                
        a: Давайте посмотрим погоду в другом месте?
        buttons: 
            "Прогноз в другом месте"
            "Не нужен прогноз"
    #другое место очищаем место, координаты и дату, идем на начало прогноза
